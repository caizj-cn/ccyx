cc.Class({
    ...
    start:function(){
        if(cc.jzc.gameNetMgr.dissoveData){
            this.showDissolveNotice(cc.jzc.gameNetMgr.dissoveData);
        }
    },
    ...
    onBtnClicked:function(event){
        ...
        if(btnName == "btn_agree"){
            cc.jzc.net.send("dissolve_agree");
        }
        else if(btnName == "btn_reject"){
            cc.jzc.net.send("dissolve_reject");
        }
        else if(btnName == "btn_sqjsfj"){
            cc.jzc.net.send("dissolve_request"); 
        }
    },
    ...
    showDissolveNotice:function(data){
        ...
        for(var i = 0; i < data.states.length; ++i){
            var b = data.states[i];
            var name = cc.jzc.gameNetMgr.seats[i].name;
            if(b){
                this._extraInfo += "\n[已同意] "+ name;
            }
            else{
