cc.Class({
    ...
    onLoad: function () {
        ...
        this._huanpaitip.active = cc.jzc.gameNetMgr.isHuanSanZhang;
        
        if(this._huanpaitip.active){
            this.showHuanpai(cc.jzc.gameNetMgr.getSelfData().huanpais == null);
        }
        this.initHuaipaiInfo();
        
        var btnOk = cc.find("Canvas/huansanzhang/btn_ok");
        if(btnOk){
            cc.jzc.utils.addClickEvent(btnOk,this.node,"HuanSanZhang","onHuanSanZhang");
        }
        ...
        this.node.on('huanpai_notify',function(data){
            if(data.detail.seatindex == cc.jzc.gameNetMgr.seatIndex){
    ...
    initHuaipaiInfo:function(){
        ...
        var seat = cc.jzc.gameNetMgr.getSelfData();
        ...
        for(var i = 0; i < seat.huanpais.length; ++i){
            huaipaiinfo.getChildByName("hp" + (i + 1)).getComponent(cc.Sprite).spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID("M_",seat.huanpais[i]);
        }
        
        var hpm = huaipaiinfo.getChildByName("hpm");
        hpm.active = true;
        if(cc.jzc.gameNetMgr.huanpaimethod == 0){
            hpm.rotation = 90;
        }
        else if(cc.jzc.gameNetMgr.huanpaimethod == 1){
            hpm.rotation = 0;
        }
        else if(cc.jzc.gameNetMgr.huanpaimethod == 2){
            hpm.rotation = 180;
        }
        else{
            hpm.active = false;
    ...
    onHuanSanZhang:function(event){
        ...
        for(var i = 0; i < this._huanpaiArr.length; ++i){
            ...
            var nt = cc.jzc.mahjongmgr.getMahjongType(pai); 
            if(type == null){
                type = nt;
            }
            else{
