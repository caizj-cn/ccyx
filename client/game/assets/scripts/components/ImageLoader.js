function loadImage(url,code,callback){
    /*
    if(cc.jzc.images == null){
        cc.jzc.images = {};
    }
    var imageInfo = cc.jzc.images[url];
    if(imageInfo == null){
        imageInfo = {
            image:null,
            queue:[],
        };
        cc.jzc.images[url] = imageInfo;
    }
...
function getBaseInfo(userid,callback){
    if(cc.jzc.baseInfoMap == null){
        cc.jzc.baseInfoMap = {};
    }
    
    if(cc.jzc.baseInfoMap[userid] != null){
        callback(userid,cc.jzc.baseInfoMap[userid]);
    }
    else{
        cc.jzc.http.sendRequest('/base_info',{userid:userid},function(ret){
            var url = null;
            if(ret.headimgurl){
            ...
            cc.jzc.baseInfoMap[userid] = info;
            callback(userid,info);
            
        },cc.jzc.http.master_url);   
    }  
};
