cc.Class({
    ...
    initView:function(){
        ...
        this.queYiMen.active = cc.jzc.gameNetMgr.isDingQueing;
        ...
        if(cc.jzc.gameNetMgr.gamestate == "dingque"){
    ...
    initEventHandlers:function(){
        ...
        this.node.on('game_dingque_notify',function(data){
            var seatIndex = cc.jzc.gameNetMgr.getSeatIndexByID(data.detail);
        ...
        this.node.on('game_dingque_finish',function(){
            ...
            cc.jzc.gameNetMgr.isDingQueing = false;
            self.initDingQue();
        });
    },
    ...
    showDingQueChoice:function(){
        ...
        var sd = cc.jzc.gameNetMgr.getSelfData();
        var typeCounts = [0,0,0];
        for(var i = 0; i < sd.holds.length; ++i){
            var pai = sd.holds[i];
            var type = cc.jzc.mahjongmgr.getMahjongType(pai);
            typeCounts[type]++;
        }
    ...
    initDingQue:function(){
        var arr = ["tong","tiao","wan"];
        var data = cc.jzc.gameNetMgr.seats;
    ...
    onQueYiMenClicked:function(event){
        ...
        cc.jzc.gameNetMgr.dingque = type;
        cc.jzc.net.send("dingque",type);
