    ...
    createRoom:function(){
        ...
        var onCreate = function(ret){
            if(ret.errcode !== 0){
                ...
                if(ret.errcode == 2222){
                    cc.jzc.alert.show("提示","房卡不足，创建房间失败!");  
                }
                else{
                    cc.jzc.alert.show("提示","创建房间失败,错误码:" + ret.errcode);
                }
            }
            else{
        ...
        console.log(data);
        cc.jzc.wc.show("正在创建房间");
        cc.jzc.http.sendRequest("/create_private_room",data,onCreate);   
    }
