cc.Class({
    ...
    onLoad: function () {
        ...
        if(btnVoice){
            cc.jzc.utils.addClickEvent(btnVoice,this.node,"Voice","onBtnOKClicked");
        }
        ...
        if(btnVoice){
            btnVoice.on(cc.Node.EventType.TOUCH_START,function(){
                ...
                cc.jzc.voiceMgr.prepare("record.amr");
    ...
    onVoiceOK:function(){
        if(this._lastTouchTime != null){
            cc.jzc.voiceMgr.release();
            var time = Date.now() - this._lastTouchTime;
            var msg = cc.jzc.voiceMgr.getVoiceData("record.amr");
            cc.jzc.net.send("voice_msg",{msg:msg,time:time});
        }
        this._voice.active = false;
    },
    ...
    update: function (dt) {
        if(this._voice.active == true && this._voice_failed.active == false){
            if(Date.now() - this._lastCheckTime > 300){
                ...
                var v = cc.jzc.voiceMgr.getVoiceLevel(7);
                if(v >= 1 && v <= 7){
