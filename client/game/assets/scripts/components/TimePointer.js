cc.Class({
    ...
    initPointer:function(){
        ...
        this._arrow.active = cc.jzc.gameNetMgr.gamestate == "playing";
        if(!this._arrow.active){
            return;
        }
        var turn = cc.jzc.gameNetMgr.turn;
        var localIndex = cc.jzc.gameNetMgr.getLocalIndex(turn);
    ...
    update: function (dt) {
        if(this._time > 0){
            ...
            if(this._alertTime > 0 && this._time < this._alertTime){
                cc.jzc.audioMgr.playSFX("timeup_alarm.mp3");
                this._alertTime = -1;
            }
            var pre = "";
            if(this._time < 0){
