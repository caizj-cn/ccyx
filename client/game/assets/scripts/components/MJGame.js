cc.Class({
    ...
    onLoad: function () {
        ...
        this.initWanfaLabel();
        this.onGameBeign();
        cc.jzc.audioMgr.playBGM("bgFight.mp3");
    },
    ...
    initView:function(){
        ...
        this._mjcount.string = "剩余" + cc.jzc.gameNetMgr.numOfMJ + "张";
        this._gamecount = gameChild.getChildByName('gamecount').getComponent(cc.Label);
        this._gamecount.string = "" + cc.jzc.gameNetMgr.numOfGames + "/" + cc.jzc.gameNetMgr.maxNumOfGames + "局";
    ...
    initEventHandlers:function(){
        cc.jzc.gameNetMgr.dataEventHandler = this.node;
        ...
        this.node.on('game_chupai',function(data){
            ...
            self.hideChupai();
            self.checkQueYiMen();
            if(data.last != cc.jzc.gameNetMgr.seatIndex){
                self.initMopai(data.last,null);   
            }
            if(!cc.jzc.replayMgr.isReplay() && data.turn != cc.jzc.gameNetMgr.seatIndex){
        ...
        this.node.on('game_mopai',function(data){
            ...
            data = data.detail;
            var pai = data.pai;
            var localIndex = cc.jzc.gameNetMgr.getLocalIndex(data.seatIndex);
            if(localIndex == 0){
                var index = 13;
            ...
            else if(cc.jzc.replayMgr.isReplay()){
        ...
        this.node.on('hupai',function(data){
            ...
            var localIndex = cc.jzc.gameNetMgr.getLocalIndex(seatIndex);
            ...
            var seatData = cc.jzc.gameNetMgr.seats[seatIndex];
            seatData.hued = true;
            if(cc.jzc.gameNetMgr.conf.type == "xlch"){
                ...
                if(data.iszimo){
                    if(seatData.seatindex == cc.jzc.gameNetMgr.seatIndex){
                        seatData.holds.pop();
                        self.initMahjongs();                
                    }
                    else{
                ...
                {
                    //if(cc.jzc.replayMgr.isReplay() == false && localIndex != 0){
            ...
            if(cc.jzc.replayMgr.isReplay() == true && cc.jzc.gameNetMgr.conf.type != "xlch"){
                var opt = self._opts[localIndex];
                opt.node.active = true;
                opt.sprite.spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID("M_",data.hupai);                
            }
            ...
            cc.jzc.audioMgr.playSFX("nv/hu.mp3");
        });
        
        this.node.on('mj_count',function(data){
            self._mjcount.string = "剩余" + cc.jzc.gameNetMgr.numOfMJ + "张";
        });
        
        this.node.on('game_num',function(data){
            self._gamecount.string = "" + cc.jzc.gameNetMgr.numOfGames + "/" + cc.jzc.gameNetMgr.maxNumOfGames + "局";
        });
        ...
        this.node.on('game_chupai_notify',function(data){
            ...
            self.showChupai();
            var audioUrl = cc.jzc.mahjongmgr.getAudioURLByMJID(data.detail.pai);
            cc.jzc.audioMgr.playSFX(audioUrl);
        });
        ...
        this.node.on('guo_notify',function(data){
            ...
            cc.jzc.audioMgr.playSFX("give.mp3");
        });
        ...
        this.node.on('peng_notify',function(data){    
            ...
            cc.jzc.audioMgr.playSFX("nv/peng.mp3");
            self.hideOptions();
        });
        ...
        this.node.on('gang_notify',function(data){
            ...
            if(gangtype == "wangang"){
                ...
                cc.jzc.audioMgr.playSFX("guafeng.mp3");
            }
            else{
                self.playEfx(localIndex,"play_xiayu");
                cc.jzc.audioMgr.playSFX("rain.mp3");
            }
        });
        ...
        this.node.on("hangang_notify",function(data){
            ...
            cc.jzc.audioMgr.playSFX("nv/gang.mp3");
            self.hideOptions();
        });
    },
    
    showChupai:function(){
        var pai = cc.jzc.gameNetMgr.chupai; 
        if( pai >= 0 ){
            //
            var localIndex = this.getLocalIndex(cc.jzc.gameNetMgr.turn);
            var sprite = this._chupaiSprite[localIndex];
            sprite.spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID("M_",pai);
    ...
    initWanfaLabel:function(){
        ...
        wanfa.string = cc.jzc.gameNetMgr.getWanfa();
    },
        ...
        if(cc.jzc.gameNetMgr.conf.type == "xlch"){
            ...
            for(var i = 0; i < hupailist.children.length; ++i){
                ...
                if(hupainode.active == false){
                    var pre = cc.jzc.mahjongmgr.getFoldPre(localIndex);
                    hupainode.getComponent(cc.Sprite).spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID(pre,pai);
    ...
    onGameBeign:function(){
        ...
        for(var i = 0; i < cc.jzc.gameNetMgr.seats.length; ++i){
            var seatData = cc.jzc.gameNetMgr.seats[i];
            var localIndex = cc.jzc.gameNetMgr.getLocalIndex(i);        
        ...
        for(var i = 0; i < sides.length; ++i){
            ...
            for(var j = 0; j < holds.childrenCount; ++j){
                ...
                sprite.spriteFrame = cc.jzc.mahjongmgr.holdsEmpty[i+1];
            }            
        }
      
        if(cc.jzc.gameNetMgr.gamestate == "" && cc.jzc.replayMgr.isReplay() == false){
            return;
        }
        ...
        this.showChupai();
        if(cc.jzc.gameNetMgr.curaction != null){
            this.showAction(cc.jzc.gameNetMgr.curaction);
            cc.jzc.gameNetMgr.curaction = null;
        }
    ...
    onMJClicked:function(event){
        if(cc.jzc.gameNetMgr.isHuanSanZhang){
            this.node.emit("mj_clicked",event.target);
            return;
        }
        
        //如果不是自己的轮子，则忽略
        if(cc.jzc.gameNetMgr.turn != cc.jzc.gameNetMgr.seatIndex){
            console.log("not your turn." + cc.jzc.gameNetMgr.turn);
            return;
        }
    ...
    initMopai:function(seatIndex,pai){
        var localIndex = cc.jzc.gameNetMgr.getLocalIndex(seatIndex);
        var side = cc.jzc.mahjongmgr.getSide(localIndex);
        var pre = cc.jzc.mahjongmgr.getFoldPre(localIndex);
        ...
        if(pai == null){
            ...
            var sprite = nc.getComponent(cc.Sprite); 
            sprite.spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID(pre,pai);
        }
        else if(pai != null){
            nc.active = true;
            if(side == "up"){
            ...
            var sprite = nc.getComponent(cc.Sprite); 
            sprite.spriteFrame = cc.jzc.mahjongmgr.getHoldsEmptySpriteFrame(side);
        }
    },
    ...
    initEmptySprites:function(seatIndex){
        ...
        var sideChild = gameChild.getChildByName(side);
        var holds = sideChild.getChildByName("holds");
        var spriteFrame = cc.jzc.mahjongmgr.getEmptySpriteFrame(side);
    ...
    initOtherMahjongs:function(seatData){
        ...
        if(holds != null && holds.length > 0){
            for(var i = 0; i < holds.length; ++i){
                ...
                sprite.node.active = true;
                sprite.spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID(pre,holds[i]);
            }
    ...
    sortHolds:function(seatData){
        ...
        cc.jzc.mahjongmgr.sortMJ(holds,dingque);
    ...
    initMahjongs:function(){
        var seats = cc.jzc.gameNetMgr.seats;
        var seatData = seats[cc.jzc.gameNetMgr.seatIndex];
        var holds = this.sortHolds(seatData);
        if(holds == null){
            return;
        }
    ...
    setSpriteFrameByMJID:function(pre,sprite,mjid){
        sprite.spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID(pre,mjid);
        sprite.node.active = true;
    },
    
    //如果玩家手上还有缺的牌没有打，则只能打缺牌
    checkQueYiMen:function(){
        if(cc.jzc.gameNetMgr.conf==null || cc.jzc.gameNetMgr.conf.type != "xlch" || !cc.jzc.gameNetMgr.getSelfData().hued){
            //遍历检查看是否有未打缺的牌 如果有，则需要将不是定缺的牌设置为不可用
            var dingque = cc.jzc.gameNetMgr.dingque;
    //        console.log(dingque)
            var hasQue = false;
            if(cc.jzc.gameNetMgr.seatIndex == cc.jzc.gameNetMgr.turn){
                for(var i = 0; i < this._myMJArr.length; ++i){
                    var sprite = this._myMJArr[i];
    //                console.log("sprite.node.mjId:" + sprite.node.mjId);
                    if(sprite.node.mjId != null){
                        var type = cc.jzc.mahjongmgr.getMahjongType(sprite.node.mjId);
                        if(type == dingque){
                            hasQue = true;
                            break;
                        }
    ...
    getLocalIndex:function(index){
        var ret = (index - cc.jzc.gameNetMgr.seatIndex + 4) % 4;
        //console.log("old:" + index + ",base:" + cc.jzc.gameNetMgr.seatIndex + ",new:" + ret);
        return ret;
    },
    ...
    onOptionClicked:function(event){
        ...
        if(event.target.name == "btnPeng"){
            cc.jzc.net.send("peng");
        }
        else if(event.target.name == "btnGang"){
            cc.jzc.net.send("gang",event.target.pai);
        }
