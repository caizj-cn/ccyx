cc.Class({
    ...
    onLoad: function () {
        ...
        if(cc.jzc.gameNetMgr.conf == null){
            return;
        }
        if(cc.jzc.gameNetMgr.conf.type == "xzdd"){
    ...
    onGameOver_XZDD(data){
        ...
        var myscore = data[cc.jzc.gameNetMgr.seatIndex].score;
        if(myscore > 0){
        ...
        for(var i = 0; i < 4; ++i){
            ...
            seatView.username.string = cc.jzc.gameNetMgr.seats[i].name;
            seatView.zhuang.active = cc.jzc.gameNetMgr.button == i;
            ...
            cc.jzc.mahjongmgr.sortMJ(userData.holds,userData.dingque);
            ...
            for(var j = 0; j < userData.huinfo.length; ++j){
                ...
                if(info.ishupai){
                    if(hi < hupaiRoot.children.length){
                        ...
                        hupaiView.getComponent(cc.Sprite).spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID("B_",info.pai);
                        hi++;   
                    }
                }
    ...
    initPengAndGangs:function(seatView,index,mjid,flag){
        ...
        if(seatView._pengandgang.length <= index){
            pgroot = cc.instantiate(cc.jzc.mahjongmgr.pengPrefabSelf);
        ...
        for(var s = 0; s < sprites.length; ++s){
            ...
            if(sprite.node.name == "gang"){
                ...
                if(flag == "angang"){
                    sprite.spriteFrame = cc.jzc.mahjongmgr.getEmptySpriteFrame("myself");
                    sprite.node.scaleX = 1.4;
                    sprite.node.scaleY = 1.4;                        
                }   
                else{
                    sprite.spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID("B_",mjid);    
                }
            }
            else{ 
                sprite.spriteFrame = cc.jzc.mahjongmgr.getSpriteFrameByMJID("B_",mjid);
