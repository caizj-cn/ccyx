    ...
    takeAction:function(){
        ...
        if(this._lastAction != null && this._lastAction.type == ACTION_CHUPAI){
            if(action != null && action.type != ACTION_PENG && action.type != ACTION_GANG && action.type != ACTION_HU){
                cc.jzc.gameNetMgr.doGuo(this._lastAction.si,this._lastAction.pai);
            }
        }
        ...
        if(action.type == ACTION_CHUPAI){
            //console.log("chupai");
            cc.jzc.gameNetMgr.doChupai(action.si,action.pai);
            return 1.0;
        }
        else if(action.type == ACTION_MOPAI){
            //console.log("mopai");
            cc.jzc.gameNetMgr.doMopai(action.si,action.pai);
            cc.jzc.gameNetMgr.doTurnChange(action.si);
            return 0.5;
        }
        else if(action.type == ACTION_PENG){
            //console.log("peng");
            cc.jzc.gameNetMgr.doPeng(action.si,action.pai);
            cc.jzc.gameNetMgr.doTurnChange(action.si);
            return 1.0;
        }
        else if(action.type == ACTION_GANG){
            //console.log("gang");
            cc.jzc.gameNetMgr.dispatchEvent('hangang_notify',action.si);
            cc.jzc.gameNetMgr.doGang(action.si,action.pai);
            cc.jzc.gameNetMgr.doTurnChange(action.si);
            return 1.0;
        }
        else if(action.type == ACTION_HU){
            //console.log("hu");
            cc.jzc.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false});
            return 1.5;
        }
    }
